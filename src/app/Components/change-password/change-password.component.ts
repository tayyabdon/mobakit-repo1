...
export class ChangePasswordComponent implements OnInit {
  ...
  ChangePassword(regForm:any){
    ...
    if(this.model.Password!=this.model.ConfirmPassword){
      ...
      this.http.post('http://test-load-balancer-1828335635.us-east-1.elb.amazonaws.com/changePassword',JSON.stringify(this.model),{headers: new HttpHeaders({'Content-Type': 'application/json'})}).subscribe((res)=>{
        this.result=res
