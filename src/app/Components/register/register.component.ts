...
export class RegisterComponent implements OnInit {
  ...
  RegisterCustomer(regForm: any) {
    if (this.model.Phone.length < 10 || this.model.Password != this.model.ConfirmPassword) {
      ...
      this.http.post('http://mobikart-alb-1432438635.us-east-1.elb.amazonaws.com/processRegister', data, { headers: new HttpHeaders({ 'Content-Type': 'application/json' }) }).subscribe((res) => {
