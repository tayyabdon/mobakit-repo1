...
export class ChangePasswordComponent implements OnInit {
  ...
  ChangePassword(regForm: any) {
    ...
    if (this.model.Password != this.model.ConfirmPassword) {
      ...
      this.http.post('http://mobikart-alb-1432438635.us-east-1.elb.amazonaws.com/changePassword', JSON.stringify(this.model), { headers: new HttpHeaders({ 'Content-Type': 'application/json' }) }).subscribe((res) => {
        this.result = res
